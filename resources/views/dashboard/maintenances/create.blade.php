...
@section('content')
    <div class="recent-orders" style="text-align: center">
        <div class="login-card">
            <h1>إضافة صيانة جديدة</h1>
           
                <form action="{{ route('dashboard.import.maintenances') }}" method="POST" enctype="multipart/form-data">
                    @csrf
            ...
            <form action="{{ route('dashboard.maintenances.store') }}" method="POST" class="login-form">
                @csrf
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <input type="number" name="total_quantity" id="total_quantity"
                                    ...
                                    value="{{ old('total_quantity') }}" placeholder="أدخل الكمية" required>
                                ...
                                <input type="text" name="execution_sites" id="execution_sites"
                                    ...
                                    value="{{ old('execution_sites') }}" placeholder="أدخل مواقع التنفيذ" required>
                                ...
                                <input type="number" step="0.01" name="total_cost" id="total_cost"
                                    ...
                                    value="{{ old('total_cost') }}" placeholder="أدخل التكلفة" required>
                                @error('total_cost')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <textarea name="maintenance_details" id="maintenance_details"
                                    class="form-control @error('maintenance_details') is-invalid @enderror" placeholder="أدخل تفاصيل الصيانة">{{ old('maintenance_details') }}</textarea>
                                ...
                                <input type="text" name="contractor_name" id="contractor_name"
                                    ...
                                    value="{{ old('contractor_name') }}" placeholder="أدخل اسم المقاول">
                                ...
                                <input type="text" name="technician_name" id="technician_name"
                                    ...
                                    value="{{ old('technician_name') }}" placeholder="أدخل اسم الفني">
                                ...
                                <select name="status" id="status"
                                    ...
                                    <option value="قيد التنفيذ" {{ old('status') == 'قيد التنفيذ' ? 'selected' : '' }}>قيد
                                        التنفيذ</option>
                                    <option value="فشلت" {{ old('status') == 'فشلت' ? 'selected' : '' }}>فشلت</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
