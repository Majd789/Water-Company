...
@section('title', 'تعديل مهمة صيانة')
...
@section('content_header')
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">تعديل مهمة صيانة: <span class="text-primary">{{ $maintenanceTask->location }}</span></h1>
            </div>
            <div class="col-sm-6">
                ...
                <div class="card card-warning">
                    <div class="card-header">
                        <h3 class="card-title mb-0">
                            ...
                            تعديل بيانات مهمة الصيانة
                        </h3>
                    </div>

                    <form action="{{ route('dashboard.maintenance_tasks.update', $maintenanceTask->id) }}" method="POST"
                        novalidate>
                        @csrf
                        @method('PUT')
                        <div class="card-body">
                                        ...
                                        <select name="unit_id" id="unit_id" class="form-control select2" required>
                                            ...
                                            @foreach ($units as $unit)
                                                <option value="{{ $unit->id }}"
                                                    {{ old('unit_id', $maintenanceTask->unit_id) == $unit->id ? 'selected' : '' }}>
                                                    {{ $unit->unit_name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" name="technician_name" id="technician_name"
                                                class="form-control"
                                                value="{{ old('technician_name', $maintenanceTask->technician_name) }}"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="date" name="maintenance_date" id="maintenance_date"
                                                class="form-control"
                                                value="{{ old('maintenance_date', $maintenanceTask->maintenance_date) }}"
                                                required>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="text" name="location" id="location" class="form-control"
                                                value="{{ old('location', $maintenanceTask->location) }}"
                                                placeholder="مثال: محطة حيش - بئر رقم 2" required>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <textarea name="fault_description" id="fault_description" class="form-control" rows="3"
                                            placeholder="اكتب وصفاً دقيقاً للعطل" required>{{ old('fault_description', $maintenanceTask->fault_description) }}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <textarea name="fault_cause" id="fault_cause" class="form-control" rows="3"
                                            placeholder="اكتب السبب الذي تم تحديده للعطل">{{ old('fault_cause', $maintenanceTask->fault_cause) }}</textarea>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <textarea name="maintenance_actions" id="maintenance_actions" class="form-control" rows="4"
                                            placeholder="اكتب بالتفصيل الإجراءات التي تم اتخاذها" required>{{ old('maintenance_actions', $maintenanceTask->maintenance_actions) }}</textarea>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="d-flex pt-2">
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="is_fixed"
                                                    ...
                                                    {{ old('is_fixed', $maintenanceTask->is_fixed) == 1 ? 'checked' : '' }}
                                                    required>
                                            ...
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="is_fixed"
                                                    ...
                                                    {{ old('is_fixed', $maintenanceTask->is_fixed) == 0 ? 'checked' : '' }}
                                                    required>
                                    ...
                                    <div class="form-group" id="reason_not_fixed_container"
                                        style="display: {{ old('is_fixed', $maintenanceTask->is_fixed) == 0 ? 'block' : 'none' }};">
                                        ...
                                        <textarea name="reason_not_fixed" id="reason_not_fixed" class="form-control" rows="2"
                                            placeholder="اكتب سبب عدم اكتمال الإصلاح">{{ old('reason_not_fixed', $maintenanceTask->reason_not_fixed) }}</textarea>
                                    </div>
                                </div>
                            </div>
                            {{-- 4. ملاحظات إضافية --}}
                                ...
                                <div class="col-12">
                                    <div class="form-group">
                                        ...
                                        <textarea name="notes" id="notes" class="form-control" rows="3"
                                            placeholder="أي ملاحظات إضافية حول المهمة">{{ old('notes', $maintenanceTask->notes) }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
