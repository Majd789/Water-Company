                ...
                <div class="col-12">
                    ...
                    @if (auth()->check() && (auth()->user()->role_id == 'admin' || auth()->user()->unit_id == null))
                        <div class="card card-default">
                            ...
                            <div class="card-body">
                                <form method="GET" action="{{ route('elevated-tanks.index') }}" id="unitFilterForm">
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                            ...
                            <div class="card-tools d-flex align-items-center">
                                ...
                                <a href="{{ route('elevated-tanks.export', request()->query()) }}"
                                    class="btn btn-success ml-2">
                                    <i class="fas fa-file-excel"></i> تصدير Excel
                                </a>
                                @if (auth()->check() && in_array(auth()->user()->role_id, ['admin', 'super']))
                                    <a href="{{ route('elevated-tanks.create') }}" class="btn btn-primary">
                                        ...
                                        @forelse ($elevatedTanks as $tank)
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <div class="btn-group">
                                                        <a href="{{ route('elevated-tanks.show', $tank->id) }}"
                                                            class="btn btn-sm btn-outline-info" title="عرض">
                                                            <i class="fas fa-eye"></i>
                                                        </a>
                                                        @if (auth()->check() && in_array(auth()->user()->role_id, ['admin', 'super']))
                                                            <a href="{{ route('elevated-tanks.edit', $tank->id) }}"
                                                                class="btn btn-sm btn-outline-warning" title="تعديل">
                                                                <i class="fas fa-edit"></i>
                                                            </a>
                                                        @endif
                                                        @if (auth()->check() && auth()->user()->role_id == 'admin')
                                                            <form action="{{ route('elevated-tanks.destroy', $tank->id) }}"
