...
.custom-date {
    width: 100%;
    padding-right: 40px; /* مساحة لأيقونة التقويم */
    padding-left: 12px;
    height: 38px;
    font-size: 15px;
...
@section('content')
    <div class="recent-orders" style="text-align: center">
        ...
        <a style="font-size: x-large" href="{{ route('weekly_reports.export') }}" class="btn btn-success">📥(Excel)</a>
        @if (auth()->check() && (auth()->user()->role_id == 'admin' || auth()->user()->unit_id == null))
            <form method="GET" action="{{ route('weekly_reports.index') }}" class="text-center mb-3" id="unitFilterForm">
        ...
        @endif
        <form method="GET" action="{{ route('weekly_reports.index') }}" class="d-flex justify-content-center mb-3">
            <div class="recent-orders d-flex align-items-center gap-2 p-3 rounded shadow bg-light flex-wrap">

                @if (auth()->check() && in_array(auth()->user()->role_id, ['admin', 'super', 'superA']))
                    {{-- حقل التاريخ --}}
                    ...
                    @if (auth()->check() && in_array(auth()->user()->role_id, ['admin', 'super']))
                        <a id="btnb" href="{{ route('weekly_reports.create') }}" class="btn btn-primary">
                ...
                {{-- زر الإلغاء --}}
                <a id="btnb" href="{{ route('weekly_reports.index') }}" class="btn btn-secondary">
                ...
                <tbody>
                    @foreach ($reports as $report)
                        <tr>
                            ...
                            <td>
                                <a id="show" href="{{ route('weekly_reports.show', $report->id) }}"
                                    class="btn btn-info btn-sm">
                                    <i class="fas fa-eye"></i>
                                </a>
                                @if (auth()->check() && in_array(auth()->user()->role_id, ['admin', 'super']))
                                    <a id="edit" href="{{ route('weekly_reports.edit', $report->id) }}"
                                        class="btn btn-warning btn-sm">
                                        <i class="fas fa-pencil-alt"></i>
                                    </a>
                                @endif
                                @if (auth()->check() && auth()->user()->role_id == 'admin')
                                    <form action="{{ route('weekly_reports.destroy', $report->id) }}" method="POST"
