...
@section('title', 'قائمة محطات الطاقة الشمسية')
...
@section('content')
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>قائمة محطات الطاقة الشمسية</h1>
                </div>
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}">الرئيسية</a></li>
                        <li class="breadcrumb-item active">محطات الطاقة الشمسية</li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="col-12">
                    ...
                    @if (auth()->check() && (auth()->user()->role_id == 'admin' || auth()->user()->unit_id == null))
                        <div class="card card-default">
                            ...
                            <div class="card-body">
                                <form method="GET" action="{{ route('solar_energy.index') }}" id="unitFilterForm">
                                    <div class="row align-items-end">
                                        <div class="col-md-9">
                                            <div class="form-group mb-0">
                                                <label>اختر وحدة لعرض محطاتها:</label>
                    ...
                    <div class="card card-primary card-outline">
                        <div class="card-header">
                            <h3 class="card-title">
                                ...
                                عرض المحطات <span class="badge badge-primary ml-2">{{ $solarEnergies->count() }}</span>
                            </h3>
                            <div class="card-tools d-flex align-items-center">
                                {{-- تمرير الفلاتر الحالية لزر التصدير --}}
                                <a href="{{ route('solar-energies.export', request()->query()) }}"
                                    class="btn btn-success ml-2">
                                    <i class="fas fa-file-excel"></i> تصدير Excel
                                </a>
                                @if (auth()->check() && in_array(auth()->user()->role_id, ['admin', 'super']))
                                    <a href="{{ route('solar_energy.create') }}" class="btn btn-primary">
                        ...
                        <div class="card-body">
                            ...
                            <div class="table-responsive">
                                <table id="solarTable" class="table table-bordered table-striped table-hover">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>المحطة</th>
                                    ...
                                    <tbody>
                                        @forelse ($solarEnergies as $solarEnergy)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $solarEnergy->station->station_name ?? 'N/A' }}</td>
                                                ...
                                                <td class="text-center">
                                                    <div class="btn-group">
                                                        <a href="{{ route('solar_energy.show', $solarEnergy->id) }}"
                                                            class="btn btn-sm btn-outline-info" title="عرض">
                                                            <i class="fas fa-eye"></i>
                                                        </a>
                                                        @if (auth()->check() && in_array(auth()->user()->role_id, ['admin', 'super']))
                                                            <a href="{{ route('solar_energy.edit', $solarEnergy->id) }}"
                                                        ...
                                                        @if (auth()->check() && auth()->user()->role_id == 'admin')
                                                            <form
                                                                action="{{ route('solar_energy.destroy', $solarEnergy->id) }}"
...
@push('scripts')
    ...
    <script>
        $(function() {
            ...
            // تهيئة DataTable
            var table = $("#solarTable").DataTable({
                "responsive": true,
                "lengthChange": true,
                "autoWidth": false,
                "paging": true,
            ...
            table.buttons().container().appendTo('#solarTable_wrapper .col-md-6:eq(0)');
