...
class WellController extends Controller
{
    public function index(Request $request)
    {
        ...
        // تقييد النتائج للمستخدمين غير الإداريين بحيث تُسترجع الآبار الخاصة بمحطة المستخدم فقط
        if ($user->role_id != 'admin') {
            $wellsQuery->where('station_id', $user->station_id);
        }
    
        // استرجاع الآبار مع المحطات وترقيم النتائج
        $wells = $wellsQuery->with('station')->paginate(500000);
