...
@section('content')
    <div class="recent-orders" style="text-align: center">
        ...
        <a style="font-size: x-large" href="{{ route('maintenances.export') }}" class="btn btn-success">📥 (Excel)</a>

        @if (auth()->check() && (auth()->user()->role_id == 'admin' || auth()->user()->unit_id == null))
            <form method="GET" action="{{ route('maintenances.index') }}" class="text-center mb-3" id="unitFilterForm">
        ...
        <form method="GET" action="{{ route('maintenances.index') }}" class="d-flex justify-content-center mb-3">
            <div class="recent-orders d-flex align-items-center gap-2 p-3 rounded shadow bg-light">
                @if (auth()->check() && in_array(auth()->user()->role_id, ['admin', 'super']))
                    <a id="btnb" href="{{ route('maintenances.create') }}" class="btn btn-primary">
                ...
                <a id="btnb" href="{{ route('maintenances.index') }}" class="btn btn-primary">
            ...
            <tbody>
                @foreach ($maintenances as $index => $maintenance)
                    <tr>
                        ...
                        <td>
                            <a id="show" href="{{ route('maintenances.show', $maintenance->id) }}"
                                class="btn btn-info btn-sm">
                                <i class="fas fa-eye"></i>
                            </a>
                            @if (auth()->check() && in_array(auth()->user()->role_id, ['admin', 'super']))
                                <a id="edit" href="{{ route('maintenances.edit', $maintenance->id) }}"
