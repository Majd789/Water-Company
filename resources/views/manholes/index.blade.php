...
@section('content')
    ...
    <div class="recent-orders" style="text-align: center">
        ...
        <a style="font-size: x-large" href="{{ route('manholes.export') }}" class="btn btn-success">📥(Excel)</a>
        @if (auth()->check() && (auth()->user()->role_id == 'admin' || auth()->user()->unit_id == null))
            <form method="GET" action="{{ route('manholes.index') }}" class="text-center mb-3" id="unitFilterForm">
        ...
        @endif
        <form method="GET" action="{{ route('manholes.index') }}" class="d-flex justify-content-center mb-3">
            <div class="recent-orders d-flex align-items-center gap-2 p-3 rounded shadow bg-light">
                @if (auth()->check() && in_array(auth()->user()->role_id, ['admin', 'super']))
                    <a id="btnb" href="{{ route('manholes.create') }}" class="btn btn-primary">
                ...
                <a id="btnb" href="{{ route('manholes.index') }}" class="btn btn-primary">
        ...
        <!-- عرض رسالة النجاح بعد إضافة أو تعديل أو حذف عنصر -->
        @if (session('success'))
            ...
            <tbody>
                @foreach ($manholes as $manhole)
                    <tr>
                        ...
                        <td>
                            <a id="show" href="{{ route('manholes.show', $manhole->id) }}"
                                class="btn btn-info btn-sm">
                                <i class="fas fa-eye"></i>
                            </a>
                            @if (auth()->check() && in_array(auth()->user()->role_id, ['admin', 'super']))
                                <a id="edit" href="{{ route('manholes.edit', $manhole->id) }}"
                                    class="btn btn-warning btn-sm">
                                    <i class="fas fa-pencil-alt"></i>
                                </a>
                            @endif
                            @if (auth()->check() && auth()->user()->role_id == 'admin')
                                <form action="{{ route('manholes.destroy', $manhole->id) }}" method="POST"
