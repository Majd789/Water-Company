...
@section('content_header')
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0 text-info"><i class="fas fa-user-edit ml-2"></i>تعديل الموظف: {{ $employee->full_name }}</h1>
            </div>
            <div class="col-sm-6">
            ...
            <div class="col-lg-11 col-xl-10">
                ...
                <div class="card card-info card-outline">
                    <div class="card-header">
                        <h3 class="card-title mb-0">تحديث البيانات الأساسية والإدارية</h3>
                    </div>

                    <form action="{{ route('dashboard.employees.update', $employee->id) }}" method="POST" novalidate>
                        @csrf
                        @method('PUT')
                        ...
                        <div class="card-body">
                            ...
                            {{-- 1. المعلومات الشخصية --}}
                            <h5 class="mb-3 text-info" style="border-bottom: 2px solid #17a2b8; display: inline-block; padding-bottom: 5px;">
                            ...
                            {{-- 2. البيانات الإدارية والوحدات --}}
                            <h5 class="mt-4 mb-3 text-info" style="border-bottom: 2px solid #17a2b8; display: inline-block; padding-bottom: 5px;">
                                        ...
                                        <select name="unit_id" id="unit_id" class="form-control select2" required>
                                            ...
                                            @foreach($units as $unit)
                                                <option value="{{ $unit->id }}" {{ old('unit_id', $employee->unit_id) == $unit->id ? 'selected' : '' }}>
                                                    {{ $unit->unit_name }} {{-- تأكد أن العمود في جدول الوحدات اسمه name --}}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" name="total_allowed_days" id="total_allowed_days" class="form-control"
                                                value="{{ old('total_allowed_days', $employee->total_allowed_days) }}" min="0" required>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="remaining_days">الرصيد المتبقي الحالي</label>
                                        <div class="input-group">
                                            <div class="input-group-prepend"><span class="input-group-text bg-light"><i class="fas fa-hourglass-half"></i></span></div>
                                            <input type="text" class="form-control" value="{{ $employee->remaining_days }}" readonly disabled>
                                            <small class="form-text text-muted w-100">يتم تحديث الرصيد تلقائياً بناءً على الإجازات.</small>
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="custom-control custom-switch mt-2">
                                            ...
                                            <label class="custom-control-label" for="is_active">الموظف على رأس عمله</label>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-footer text-right">
                            <button type="submit" class="btn btn-info btn-lg px-5">
                                <i class="fas fa-sync-alt ml-1"></i> حفظ التغييرات
                            </button>
