    ...
    <div class="recent-orders" style="text-align: center">
        <div class="login-card">
            ...
            <form action="{{ route('horizontal-pumps.store') }}" method="POST" class="login-form">
                    ...
                    <div class="card-box">
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <input type="text" name="pump_name" class="form-control" value="{{ old('pump_name') }}">

                                <label for="pump_status">الحالة التشغيلية</label>
                                <select name="pump_status" class="form-control">
                                    <option value="يعمل" {{ old('pump_status') == 'يعمل' ? 'selected' : '' }}>تعمل
                                    </option>
                                    <option value="متوقفة" {{ old('pump_status') == 'متوقفة' ? 'selected' : '' }}>متوقفة
                                    </option>
                                </select>

                                <label for="technical_condition">الحالة الفنية</label>
                                <input type="text" name="technical_condition" class="form-control"
                                    value="{{ old('technical_condition') }}">
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="card-body">
                                ...
                                <select name="pump_brand_model" class="form-control">
                                    ...
                                    @foreach (['HALLER & SCHNEIDER', 'German Made', 'Italian Made', 'Turkish Made', 'STANDART', 'MEZ', 'CAPRARI', 'GAMAK', 'SEMPA', 'SEVER', 'Czech Made', 'WATT', 'European', 'SKM', 'GRUNDFOS', 'MAS', 'SIEMENS', 'ROVATTI', 'Spanish Made', 'DEMAK', 'Iranian Made', 'KLN', 'ELK', 'PENTAX', 'Chinese Made', 'LOWARA', 'JET', 'FLOWSERVE', 'KSB', 'ATURIA', 'غير معروف'] as $brand)
                                        <option value="{{ $brand }}"
                                            {{ old('pump_brand_model') == $brand ? 'selected' : '' }}>{{ $brand }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                ...
                <button type="submit" class="btn btn-primary" style="margin-top: 20px;">إضافة المضخة</button>
            </form>
        </div>
    </div>
