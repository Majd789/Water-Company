...
@section('content')
    <div class="container-fluid">
        <form action="{{ route('dashboard.projects.update', $project->id) }}" method="POST" novalidate>
            @csrf
            @method('PUT')
            ...
            <!-- =================================================== -->
            <!-- بطاقة بيانات المشروع الرئيسية (معبأة بالبيانات القديمة) -->
            <!-- =================================================== -->
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="text" id="institution_ref_number" name="institution_ref_number"
                                class="form-control"
                                value="{{ old('institution_ref_number', $project->institution_ref_number) }}" required>
                        </div>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="date" id="institution_ref_date" name="institution_ref_date" class="form-control"
                                value="{{ old('institution_ref_date', $project->institution_ref_date) }}" required>
                        </div>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="text" id="hac_ref_number" name="hac_ref_number" class="form-control"
                                value="{{ old('hac_ref_number', $project->hac_ref_number) }}">
                        </div>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="date" id="hac_ref_date" name="hac_ref_date" class="form-control"
                                value="{{ old('hac_ref_date', $project->hac_ref_date) }}">
                        </div>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <select id="type" name="type" class="form-control" required>
                                <option value="تنفيذ" @if (old('type', $project->type) == 'تنفيذ') selected @endif>تنفيذ</option>
                                <option value="تقييم احتياج" @if (old('type', $project->type) == 'تقييم احتياج') selected @endif>تقييم احتياج
                                </option>
                                <option value="أخرى" @if (old('type', $project->type) == 'أخرى') selected @endif>أخرى</option>
                            </select>
                        </div>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="text" id="donor" name="donor" class="form-control"
                                value="{{ old('donor', $project->donor) }}" required>
                        </div>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="number" id="total_cost" name="total_cost" class="form-control" step="0.01"
                                value="{{ old('total_cost', $project->total_cost) }}" required>
                        </div>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="text" id="supervisor_name" name="supervisor_name" class="form-control"
                                value="{{ old('supervisor_name', $project->supervisor_name) }}" required>
                        </div>
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="text" id="supervisor_contact" name="supervisor_contact" class="form-control"
                                value="{{ old('supervisor_contact', $project->supervisor_contact) }}">
                        </div>
            ...
            <div class="card card-secondary shadow-sm">
                ...
                <div class="card-body">
                    <div id="activities_container">
                        {{-- عرض الأنشطة القديمة أو الموجودة مسبقًا --}}
                        @foreach (old('activities', $project->activities) as $index => $activity)
                                ...
                                <div class="row">
                                    <div class="col-md-4 form-group">
                                        ...
                                        <input type="text" name="activities[{{ $index }}][activity_name]"
                                            class="form-control"
                                            value="{{ $activity['activity_name'] ?? ($activity->activity_name ?? '') }}"
                                            required>
                                    </div>
                                    <div class="col-md-4 form-group">
                                        <label>القيمة ($)</label>
                                        <input type="number" name="activities[{{ $index }}][value]"
                                            class="form-control" step="0.01"
                                            value="{{ $activity['value'] ?? ($activity->value ?? '') }}" required>
                                    </div>
                                    <div class="col-md-4 form-group">
                                        <label>حالة التنفيذ</label>
                                        <input type="text" name="activities[{{ $index }}][execution_status]"
                                            class="form-control"
                                            value="{{ $activity['execution_status'] ?? ($activity->execution_status ?? '') }}"
                                            required>
                                    </div>
                                    <div class="col-md-4 form-group">
                                        <label>الوحدة</label>
                                        <select name="activities[{{ $index }}][unit_id]" class="form-control">
                                            <option value="">-- اختر وحدة --</option>
                                            @foreach ($units as $unit)
                                                <option value="{{ $unit->id }}"
                                                    @if (($activity['unit_id'] ?? ($activity->unit_id ?? null)) == $unit->id) selected @endif>
                                    ...
                                    <div class="col-md-4 form-group">
                                        <label>البلدة</label>
                                        <select name="activities[{{ $index }}][town_id]" class="form-control">
                                            <option value="">-- اختر بلدة --</option>
                                            @foreach ($towns as $town)
                                                <option value="{{ $town->id }}"
                                                    @if (($activity['town_id'] ?? ($activity->town_id ?? null)) == $town->id) selected @endif>
                                    ...
                                    <div class="col-md-4 form-group">
                                        <label>المحطة</label>
                                        <select name="activities[{{ $index }}][station_id]" class="form-control">
                                            <option value="">-- اختر محطة --</option>
                                            @foreach ($stations as $station)
                                                <option value="{{ $station->id }}"
                                                    @if (($activity['station_id'] ?? ($activity->station_id ?? null)) == $station->id) selected @endif>
            ...
            <div class="card-footer text-left">
                <button type="submit" class="btn btn-primary"><i class="fas fa-save ml-1"></i> تحديث المشروع</button>
...
@push('scripts')
    <script>
        $(document).ready(function() {
            // البدء من عدد الأنشطة الموجودة حالياً لتجنب تضارب الفهارس
            let activityIndex = {{ count(old('activities', $project->activities)) }};
            ...
            // عند النقر على زر الحذف داخل حاوية الأنشطة
