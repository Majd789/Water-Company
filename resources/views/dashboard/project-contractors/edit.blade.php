...
@section('content_header')
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">تعديل عقد: <span class="text-primary">{{ $projectContractor->contract_code }}</span></h1>
            </div>
            <div class="col-sm-6">
        ...
        <div class="row justify-content-center">
            <div class="col-lg-10">
                ...
                <div class="card card-primary">
                    ...
                    <form action="{{ route('dashboard.project-contractors.update', $projectContractor->id) }}" method="POST" novalidate>
                        @csrf
                        @method('PUT')
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="contract_code" name="contract_code"
                                               placeholder="أدخل كوداً فريداً للعقد" value="{{ old('contract_code', $projectContractor->contract_code) }}" required>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="project_id" class="form-control select2" id="project_id" required>
                                            ...
                                            @foreach ($projects as $project)
                                                <option value="{{ $project->id }}" {{ old('project_id', $projectContractor->project_id) == $project->id ? 'selected' : '' }}>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="contractor_id" class="form-control select2" id="contractor_id" required>
                                            ...
                                            @foreach ($contractors as $contractor)
                                                <option value="{{ $contractor->id }}" {{ old('contractor_id', $projectContractor->contractor_id) == $contractor->id ? 'selected' : '' }}>
                                                    {{ $contractor->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="number" step="0.01" class="form-control" id="value" name="value"
                                               placeholder="أدخل قيمة العقد بالدولار" value="{{ old('value', $projectContractor->value) }}">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="contract_date">تاريخ العقد</label>
                                        <input type="date" class="form-control" id="contract_date" name="contract_date" value="{{ old('contract_date', $projectContractor->contract_date) }}">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="number" class="form-control" id="duration_days" name="duration_days"
                                               placeholder="مثال: 60" value="{{ old('duration_days', $projectContractor->duration_days) }}">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="start_date">تاريخ بداية التنفيذ</label>
                                        <input type="date" class="form-control" id="start_date" name="start_date" value="{{ old('start_date', $projectContractor->start_date) }}">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="end_date">تاريخ نهاية التنفيذ</label>
                                        <input type="date" class="form-control" id="end_date" name="end_date" value="{{ old('end_date', $projectContractor->end_date) }}">
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <select name="contractor_status_id" class="form-control select2" id="contractor_status_id" required>
                                            <option value="" disabled>-- اختر حالة التنفيذ --</option>
                                            @foreach ($contractorStatuses as $status)
                                                <option value="{{ $status->id }}" {{ old('contractor_status_id', $projectContractor->contractor_status_id) == $status->id ? 'selected' : '' }}>
                                                    {{ $status->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="org_approval_number" name="org_approval_number"
                                               placeholder="أدخل رقم الموافقة" value="{{ old('org_approval_number', $projectContractor->org_approval_number) }}">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="org_approval_date">تاريخ كتاب الموافقة</label>
                                        <input type="date" class="form-control" id="org_approval_date" name="org_approval_date" value="{{ old('org_approval_date', $projectContractor->org_approval_date) }}">
                                    </div>
                                </div>
                            </div>
                        </div>
